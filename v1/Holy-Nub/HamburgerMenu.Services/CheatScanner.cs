...
public static class CheatScanner
{
	...
	public static async Task<List<CheatScanResult>> ScanDirectoryAsync(string path, IProgress<string>? progress = null, CancellationToken cancellationToken = default(CancellationToken), Action<CheatScanResult>? onResultFound = null)
	{
		...
		await Task.Run(delegate
		{
			try
			{
				List<string> files = EnumerateFilesSafe(path, cancellationToken).ToList();
				progress?.Report($"Найдено {files.Count} файлов для проверки");
				...
				Parallel.ForEach(files, parallelOptions, delegate(string file)
				{
					...
					try
					{
						int num = Interlocked.Increment(ref processed);
						if (num % 100 == 0)
						{
							progress?.Report($"Проверено: {num}/{files.Count} | Найдено: {results.Count}");
						}
						...
						if (cheatScanResult != null && cheatScanResult.Confidence >= 70)
						{
							...
							onResultFound?.Invoke(cheatScanResult);
							progress?.Report($"\ud83d\udd34 {cheatScanResult.CheatName} | Всего: {results.Count}");
						}
					}
					catch
					{
	...
	private static IEnumerable<string> EnumerateFilesSafe(string path, CancellationToken cancellationToken = default(CancellationToken))
	{
